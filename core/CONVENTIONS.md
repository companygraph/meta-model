...
Validation is agent-run. Invoke it in prose — *"check cross-references in this repository"* — and the rules below are what is being checked. Some of them can also be checked by a script, but whether such a script exists is a property of the repository, not of these rules. R0 says which ones the CompanyGraph repository checks that way.
