...
// Obsidian's own URL for a folder, which opens it there and registers it as a vault as well, so
// "Open folder as vault" stops being a step.
export const vaultUrl = (vault) => `obsidian://open?path=${encodeURIComponent(resolve(vault))}`;

// The vault opened through that URL. Each platform has its own opener, handed the URL as one
