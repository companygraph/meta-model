...
export function workspaceOf({ file, plugins }) {
  ...
  const right = [
    ...
    leaf("companygraph-brief", {}, "notebook-pen", "CompanyGraph brief"),
  ];
...
// Where Obsidian keeps its list of vaults, outside every vault, at its own place per platform:
// the first that exists, or the first of the platform where none does yet. Obsidian opens by URL
// only a folder on that list, and neither the URL nor Obsidian's own command line puts one there.
function vaultListOf({ platform, env, exists }) {
  ...
  const paths = places.map((place) => join(place, "obsidian.json"));
  return { path: paths.find(exists) ?? paths[0] ?? null, folder: places[0] ?? null, join };
}

// The folder as the list would name it: its real path on the platform this runs on, since
// Obsidian's file dialog hands it the real one and `/tmp/x` and `/private/tmp/x` would otherwise
// list the same folder twice; resolved only where it does not exist yet; and as given when another
// platform is asked about, since a Windows path cannot be resolved from a Mac.
const listedAs = (vault, platform) => {
  if (platform !== process.platform) return vault;
  try {
    return realpathSync.native(vault);
  } catch {
    return resolve(vault);
  }
};

// Whether Obsidian knows a folder as a vault. A list that cannot be read means not known, which
// is the answer that costs least when wrong.
export function knownVault(vault, { platform = process.platform, env = process.env, exists = existsSync, read = (p) => readFileSync(p, "utf8") } = {}) {
  const { path } = vaultListOf({ platform, env, exists });
  if (!path || !exists(path)) return false;
  try {
    const { vaults } = JSON.parse(read(path));
    return Object.values(vaults ?? {}).some((entry) => entry?.path === listedAs(vault, platform));
  } catch {
    return false;
  }
}

// A folder put on Obsidian's list, so the URL can open it: one entry added under an id of sixteen
// hex digits as Obsidian's are, every other entry kept as it was, the list made where Obsidian has
// none yet. This is the one write into a file Obsidian treats as its own, and the command makes
// it only while Obsidian is not running: Obsidian reads the list when it starts and holds it in
// memory after, so an entry written under a running Obsidian is not seen and may be written over
// when it quits. A list that cannot be read is written fresh with this one entry.
export function registerVault(vault, { platform = process.platform, env = process.env, exists = existsSync, read = (p) => readFileSync(p, "utf8"), write = writeWhole, mkdir = (p) => mkdirSync(p, { recursive: true }) } = {}) {
  const { path, folder } = vaultListOf({ platform, env, exists });
  if (!path) throw new Error("no home folder is known, so Obsidian's list of vaults cannot be found");
  let list = { vaults: {} };
  if (exists(path)) {
    try {
      list = JSON.parse(read(path));
      if (typeof list !== "object" || list === null || Array.isArray(list)) list = { vaults: {} };
      list.vaults ??= {};
    } catch {
      list = { vaults: {} };
    }
  } else mkdir(folder);
  const target = listedAs(vault, platform);
  if (Object.values(list.vaults).some((entry) => entry?.path === target)) return path;
  list.vaults[randomBytes(8).toString("hex")] = { path: target, ts: Date.now() };
  write(path, JSON.stringify(list));
  return path;
}

// The list is Obsidian's only one, so it is written whole or not at all: to a file beside it,
// then moved over it, which the filesystem does in one step.
function writeWhole(path, text) {
  writeFileSync(`${path}.tmp`, text);
  renameSync(`${path}.tmp`, path);
}

// Obsidian quit the way its own menu quits it, an AppleScript quit, taskkill without /F, or a
// SIGTERM, never a kill, and waited for until the process list no longer names it, since it
// writes its list of vaults on the way out and an entry written before that would be lost. It
// reopens whatever was open when it quit, so nothing of the person's is lost by it. `quit` is
// false where it is still there after the wait, or the asking could not be run, and `reason` says
// why where the asking said: on macOS an Apple event needs the terminal's Automation permission,
// and a refusal names the Privacy & Security switch on stderr, which is worth more than a wait.
// On Windows taskkill answers 1 for the windowless helpers even as the window closes, so there
// the wait decides.
export async function quitObsidian({ platform = process.platform, run = spawnSync, running = () => obsidianRunning({ platform, run }), sleep = (ms) => new Promise((done) => setTimeout(done, ms)), tries = 40 } = {}) {
  const [command, ...args] = platform === "darwin" ? ["osascript", "-e", 'quit app "Obsidian"'] : platform === "win32" ? ["taskkill", "/IM", "Obsidian.exe"] : ["pkill", "-x", "obsidian"];
  const asked = run(command, args, { encoding: "utf8", stdio: ["ignore", "ignore", "pipe"] });
  if (asked.error) return { quit: false, reason: `${command} could not be run: ${asked.error.message}` };
  if (platform === "darwin" && asked.status !== 0) return { quit: false, reason: (asked.stderr ?? "").trim() || `${command} answered ${asked.status}` };
  for (let i = 0; i < tries; i++) {
    if (!running()) return { quit: true };
    await sleep(250);
  }
  return { quit: false };
}

// Whether Obsidian is running, asked of the platform's own process list. A lister that cannot be
// run or answers nothing says not running, and the opener then answers for itself.
export function obsidianRunning({ platform = process.platform, run = spawnSync } = {}) {
  const [command, ...args] = platform === "win32" ? ["tasklist", "/FI", "IMAGENAME eq Obsidian.exe", "/FO", "CSV", "/NH"] : ["pgrep", "-x", platform === "darwin" ? "Obsidian" : "obsidian"];
  const result = run(command, args, { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] });
  if (result.error || result.status !== 0) return false;
  return platform === "win32" ? /"Obsidian\.exe"/.test(result.stdout ?? "") : (result.stdout ?? "").trim().length > 0;
}
