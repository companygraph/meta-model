...
// and the folder the files go in; `repo` is where the releases are. Claudian is Claude Code in a
// pane, and asks for Claude Code on the machine; Terminal is a shell in a pane.
export const PLUGINS = [
  { id: ID, name: "CompanyGraph", repo: "companygraph/obsidian-plugin" },
  { id: "realclaudian", name: "Claudian", repo: "yishentu/claudian" },
  { id: "terminal", name: "Terminal", repo: "polyipseity/obsidian-terminal" },
];
const OWN = PLUGINS[0];
...
export function whereObsidian({ platform = process.platform, env = process.env, exists = existsSync } = {}) {
  ...
  if (platform === "darwin") {
    ...
    app = first(env.LOCALAPPDATA && join(env.LOCALAPPDATA, "Obsidian", "Obsidian.exe"));
    if (!app && onPath("winget.exe")) installer = { name: "winget", command: ["winget", "install", "--id", "Obsidian.Obsidian", "-e"] };
  } else {
    app = first(
      onPath("obsidian"),
...
// The vault opened in Obsidian through Obsidian's own URL, which registers the vault there as
// well, so "Open folder as vault" stops being a step. Each platform has its own opener, handed
// the URL as one argument and never through a shell string.
