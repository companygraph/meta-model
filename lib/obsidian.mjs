...
// The folder as the list would name it: resolved on the platform this runs on, and as given when
// another platform is asked about, since a Windows path cannot be resolved from a Mac.
const listedAs = (vault, platform) => (platform === process.platform ? resolve(vault) : vault);
...
export function registerVault(vault, { platform = process.platform, env = process.env, exists = existsSync, read = (p) => readFileSync(p, "utf8"), write = writeFileSync, mkdir = (p) => mkdirSync(p, { recursive: true }) } = {}) {
...
// reopens whatever was open when it quit, so nothing of the person's is lost by it. False where
// it is still there after the wait, or the asking could not be run.
export async function quitObsidian({ platform = process.platform, run = spawnSync, running = () => obsidianRunning({ platform, run }), sleep = (ms) => new Promise((done) => setTimeout(done, ms)), tries = 40 } = {}) {
  ...
  const asked = run(command, args, { stdio: "ignore" });
