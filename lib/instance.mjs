...
export function parseInstance(files, { sub = "" } = {}) {
  ...
  for (const e of entities) {
    ...
    for (const s of e.sections) {
      ...
      //
      // Exactly one thing changes, and it is narrow: R4 never caught a typo in one cell. The
      // first resolving cell of a row becomes the edge, so a misspelled skill beside a correct
      // Level resolves on the Level. What it catches is a row where nothing at all resolves,
      // and that stays caught wherever the table has a reference in it.
