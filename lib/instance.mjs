    ...
    for (const s of e.sections) {
      ...
      for (const row of s.table.rows) {
        ...
        s.table.columns.forEach((col, i) => {
          const cell = row[i] ?? "";
          if (col === refName) { to = resolve(refDecl, cell, where); return; }
...
// frontmatter field to its declaration and `tables` maps a section heading to the
// declarations of its columns. A declaration is `{ form, target }` where `form` is `ref`,
// `ref?` or `qualifier`; a field declared any other way is absent, which is what makes it a
// fact (R16). This walks the same Frontmatter and captioned Sections tables parseSchemas
// walks to build its edges — that walk is still its own, since one builds edges of a graph and
// the other builds declarations an instance resolves against — but every Type cell in both
// passes through the one declarationOf above, so the two readings of a cell can never disagree.
