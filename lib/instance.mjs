...
export function parseInstance(files, { sub = "", schemas } = {}) {
  ...
  const ownerTypes = new Map(
    parseSchemas(schemas).entities.filter((e) => e.fields.owner).map((e) => [e.id.slice("core/".length), e.fields.owner]),
  );
...
// vocabulary: every declared reference with how many of it a page may hold, the joins, and the
// list sections with their kind. It says only what a rule states and a check holds. A
...
export function constraintsOf(files, { sub = "" } = {}) {
  ...
  for (const e of parseSchemas(files, { sub }).entities) {
    ...
    const reference = (via, row, columns, ofPage) => {
      ...
      references.push({ via, form: decl.form, target: decl.target, array, required,
    ...
    if (frontmatter) {
      ...
      if (k >= 0) for (const row of frontmatter.rows) reference(bare(row[k]), row, frontmatter.columns, true);
    }
    ...
    out[e.id.slice("core/".length)] = { references, joins, lists };
  }
  return out;
}
