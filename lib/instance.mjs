...
function schemaTypesOf(schemas) {
  const declaredTypes = new Set(), ownerTypes = new Map();
  for (const e of parseSchemas(schemas).entities) {
    const type = e.id.slice("core/".length);
    declaredTypes.add(type);
    if (e.fields.owner) ownerTypes.set(type, e.fields.owner);
  }
  return { declaredTypes, ownerTypes };
}
export function ownerTypesOf(schemas) {
...
function ownedDirOf(path) {
  const parts = path.split("/");
  const slug = parts[parts.length - 1].replace(/\.md$/, "");
  return parts[parts.length - 2] === slug ? parts.slice(0, -1).join("/") : null;
}
...
// for the owner it named — its own `id` where the owner entity carries one, its own `path`
// otherwise — or `null` when `type` is unowned. On failure: `error` is the R4 text this rule has
// always thrown, minus the quoted subject and the `in <page>` that `resolveBy` below adds back,
// and `subject` says which of the caller's own values that quote would have been — `"value"`,
// the name being resolved, for every case but one, where the row's own owner cell fails to
// resolve before a name is even looked for, and the message quotes that instead.
function scopeOf(entities, { ownerTypes, declaredTypes }, type, owner) {
  ...
  const of = (t) => entities.filter((e) => e.type === t);
  if (!ownerType) {
    if (owner)
  ...
  const ownerId = ownerEntity.id ?? ownerEntity.path;
  const ownedDir = ownedDirOf(ownerEntity.path);
...
function resolveWithin(entities, precomputed, type, name, owner) {
...
export function rowScope(entities, schemas, { type, owner } = {}) {
...
export function resolveRow(entities, schemas, { type, name, owner } = {}) {
...
export function constraintsOf(files, { sub = "" } = {}) {
  const bare = (cell) => (cell ?? "").replace(/`/g, "").trim();
  const yes = (cell) => bare(cell) === "Yes";
  const out = {};
  for (const e of parseSchemas(files, { sub }).entities) {
    const references = [], enums = [], joins = [], lists = [];
    const reference = (via, row, columns, ofPage) => {
