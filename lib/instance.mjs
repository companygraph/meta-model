...
export function parseInstance(files, { sub = "" } = {}) {
  ...
  // it recognizes a reference by the value happening to be a canonical name. It therefore
  ...
  for (const e of entities) {
    ...
    for (const s of e.sections) {
      if (!s.table) continue;
      // Whether a row that resolves to nothing is an error is decided by the table it sits in,
      // not by the row. A table of references to the outside world — a register entry, a
      // recording — resolves to nothing anywhere, and is a table of data: it draws no edges and
      // its cells stay what they are. A table where something does resolve is a table of
      // references, and there a row resolving to nothing is the R4 it always was, because the
      // page must not draw a line to nowhere.
      //
      // Exactly one thing changes, and it is narrow: R4 never caught a typo in one cell. Every
      // resolving cell of a row draws its own edge, so a misspelled skill beside a correct
      // Level still draws the Level's edge. What it catches is a row where nothing at all
      // resolves, and that stays caught wherever the table has a reference in it.
      const resolves = s.table.rows.some((row) =>
        s.table.columns.some((_, i) => one(row[i] ?? "", e.path)));
      ...
      for (const row of s.table.rows) {
        ...
        // rated at a level names two entities, so the level a profile claims was reachable only
        // by reading a string out of the skill edge's attrs — declared, required, and drawn
        // nowhere. Each edge carries the row's other cells, so a reader who had the level in
        // attrs still has it.
