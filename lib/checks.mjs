...
import { listsDeclarationOf, underDeclarationOf, listKindOf, enumTokensOf, IMAGE_FILE, bytesOf, declarationOf } from "./instance.mjs";
...
export function imageInfoOf(bytes) {
  ...
  if (b.length >= 24 && PNG.every((x, i) => b[i] === x) && IHDR.every((x, i) => b[12 + i] === x))
...
export function instanceChecks({ files, core = "core", model = MODEL, fail }) {
  ...
  // An image the container may hold beside a page: a file of that name, never a folder. `ls`
  // names folders too, and a folder named like an image holds files the image check never
  // sees, so the name alone would let anything in.
  const isImage = (rel) => IMAGE_FILE.test(rel) && files.has(rel);
  ...
  return [
  {
    ...
    run() {
      ...
      // file, the README that is never an entity (R6), or an image the folder of a singular
      // type's page — the container itself — names (R9). Nothing else — which is the whole
      ...
      for (const entry of top)
        ...
        if (!allowed.has(entry) && !isImage(`${EX}/${entry}`))
          fail(`${EX}/${entry} is not a folder of any type (expected one of ${[...allowed].join(", ")})`);

      // A singular has no folder and so no README saying what its file is written against, which
      // a plural's has; an upgrader who meets this line learns the shape's place from it instead.
      for (const { type, file } of SINGULAR)
        if (!top.includes(file)) fail(`${EX}/${file} is missing — a singular type's entity, written against ${core}/${type}-schema.md`);
      ...
      for (const { type, folder, owns } of PLURAL) {
        ...
        for (const name of ls(`${EX}/${base}`) ?? []) {
          ...
          if (!owns) {
            // An image sits beside the page that names it (R9); whether one does is the image
            // check's finding, not this one's.
            if (!name.endsWith(".md") && !isImage(`${EX}/${base}/${name}`)) fail(`${EX}/${base}/${name} should be a .md file`);
            continue;
          }
          ...
          for (const entry of inside)
            if (entry !== `${name}.md` && entry !== "README.md" && !ownedFolders.includes(entry) && !isImage(`${EX}/${base}/${name}/${entry}`))
  ...
  },
  {
    ...
    run() {
      ...
      const held = (child, where, declared, value) => {
        ...
        // equal an H1 is a string. A `ref → by <Column>` reads its type off the row, not off
        // this declaration, so it steps aside for `heldBy` in the row loop below.
        if (!ref || ref.by) return;
      ...
      const heldBy = (child, section, column, ref, cellOf, value) => {
        ...
        if (!type) return fail(`${child}: ${where} says "${value}", and \`${ref.by}\` in "## ${section}" is blank; a name with no type cannot resolve (R4)`);
  ...
  },
  {
    ...
    // scope to be held to and is not read here; core declares one, the `by … in` form
    // (`ref → by <Column> in <Owner>`, R4, R9), and it is held by its own reader, `resolveBy` in
    // the parser and `heldBy` below, not this check.
  ...
  {
    // R15's second half. A key written twice holds two values, and two readers took two of them:
    // the checks read the first and the parser the last, so `nature: human` over `nature: robot`
    // passed every check while the sites served robot. A merge that keeps both lines, or a block
    // of fields pasted over a page that has one of them, is how the second arrives.
    name: "a frontmatter key appears once",
    rule: "R15",
    run() {
      walkMd(EX, (child, text) => {
        if (!typeOfFile(child)) return;
        ...
        for (const [key, count] of seen)
          if (count > 1) fail(`${child}: frontmatter field \`${key}\` is written twice; a key appears once (R15)`);
      });
    },
  },
  {
  ...
  },
  {
    ...
    //
    // Presence is tested, and so is a scalar left blank: `unit: ` with nothing after it, or a
    // quoted empty string, `unit: ""` or `unit: ''`, answers a required field the way an absent
    // key does, and a reader can no more use one than the other. A list field is not held to
    // blank here — `fieldValues` already reads it in every shape it may be written, and
    // "required list fields carry at least one item" is the check that reports a present-but-
    // empty list, so testing it here too would print the same defect under a second message.
    // Presence itself is still tested on the key alone, list or not, so a field a schema later
    // retypes to a list keeps being found present by this half and is not left unchecked by
    // either half.
    ...
    run() {
      ...
      walkMd(EX, (child, text) => {
        ...
        for (const { field, declared, description } of fields) {
          const name = field.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
          const present = new RegExp(`^${name}:`, "m").test(fmText);
          const isList = /^array\b/.test(declared);
          if (present && (isList || fieldValues(fmText, field).length > 0)) continue;
          const tokens = declared === "enum" ? enumTokensOf(description) : [];
          const named = tokens.length ? ` — one of ${tokens.map((v) => `\`${v}\``).join(", ")}` : "";
          const what = present ? `\`${field}\` is blank` : `no \`${field}\``;
          fail(`${child}: ${what}, which ${typeOfFile(child)}-schema.md requires${named}`);
        }
      });
    },
  },
    ...
    run() {
      ...
      walkMd(EX, (child, text) => {
        ...
        for (const field of fields) {
          const value = fmScalar(fmText, field);
          if (value === null || value === "") {
            // A block list reads as no scalar at all, and would otherwise pass in silence (a
            // flow list reads as one, fails below on its extension, and the parser refuses it
            // under R11): `fieldValues` reads the same field in every shape fmScalar does not, so
            // a field with values there and none here is a list, not an absent field. Its
            // entries still name files, so a plain one — no path in it — is added to `named`
            // before the loop below, else the file it points at would also be reported unnamed.
            const listed = fieldValues(fmText, field);
            if (listed.length) {
              fail(`${child}: \`${field}\` is a list; an image names one file (R9)`);
              for (const name of listed) if (!name.includes("/")) named.add(`${dir}/${name}`);
            }
            continue;
          }
          if (value.includes("/")) {
          ...
          // Text here is the reader's mistake and not the file's: a reader reads what
          // IMAGE_FILE matches as bytes, and one that did not has corrupted the picture already.
          if (!bytesOf(bytes)) {
            fail(`${at}: was read as text, not bytes — a reader reads what IMAGE_FILE matches as bytes (R9)`);
            continue;
          }
          ...
          const size = bytesOf(bytes).length;
          if (size > cap)
            fail(`${at}: is ${size} bytes; an image is at most ${cap} (R9)`);
        }
      });
      for (const path of [...files.keys()].sort())
