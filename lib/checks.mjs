  ...
  return [
  {
    ...
    run() {
      ...
      for (const entry of top)
        ...
        if (!allowed.has(entry) && !IMAGE_FILE.test(entry))
    ...
    run() {
      ...
      walkMd(EX, (child, text) => {
        ...
        for (const field of fields) {
          const value = fmScalar(fmText, field);
          if (value === null || value === "") {
            // A block or flow list reads as no scalar at all, and would otherwise pass in
            // silence: `fieldValues` reads the same field in every shape fmScalar does not, so
