...
import { listsDeclarationOf, underDeclarationOf, listKindOf } from "./instance.mjs";
          ...
          blocksOf(sectionsOf(read(`${core}/${t.type}-schema.md`) ?? "").get("Sections") ?? "")
            ...
            .map((b) => ({
              ...
              columns: b.table.rows.map((r) => {
                ...
                // A column typed enum lists its permitted values in its own Description, a
                // schema fact read once here rather than once per cell a walk over every
                // instance file would otherwise re-read it for. A schema whose enum opens
                // with no readable list is reported here, at the schema, and `tokens` stays
                // empty so the cell loop below knows the schema already answered for it.
                const tokens = declared === "enum" ? enumTokensOf((r[3] ?? "").trim()) : null;
                if (declared === "enum" && !tokens.length)
                  enumOpensWithNoList(`${core}/${t.type}-schema.md`, `\`${name}\` in "## ${b.section}"`);
                return {
                  name,
                  required: r[1].replace(/`/g, "").trim() === "Yes",
                  declared,
          ...
          for (const row of table.rows)
            columns.forEach((col, n) => {
              ...
              if (col.declared === "enum") {
                // A schema whose enum lists no tokens already failed once, above, where
                // columnTables was built; col.tokens is empty exactly there, and there is
                // nothing left to hold a cell to, so this returns in silence rather than
                // saying the same schema fact again for every row that reaches it.
