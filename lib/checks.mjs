...
export function instanceChecks({ files, core = "core", model = MODEL, fail }) {
  ...
  },
  {
    ...
    // R16 cannot hold that: it lands a reference on its declared type, and an owned type's names
    // run across every owner, so one profile's evidence naming another profile's experience
    // resolves, draws or qualifies an edge, and is wrong.
    //
    ...
    run() {
      ...
      walkMd(EX, (child, text) => {
        ...
        for (const decl of declared.get(type) ?? []) {
          ...
          // The scope is known where the entity is the owner, or is owned by the same owner.
          if (type !== ownerType && TYPES.find((t) => t.type === type)?.owner !== ownerType) continue;
          const rel = child.slice(EX.length + 1).split("/");
          if (rel[0] !== ownerSpec.folder.split("/")[0] || rel.length < 3) continue;
          const folder = `${EX}/${rel[0]}/${rel[1]}/${ownedSpec.folder.split("/").pop()}`;
          ...
          for (const value of new Set(values))
            if (!own(folder).has(value))
              fail(`${child}: ${where(value)} says "${value}", which is not one of ${whose}: it is the H1 of no ${decl.target} in ${folder}/, and a ${decl.target} is named only within the ${ownerType} that owns it (R5)`);
        }
      });
    },
  },
  {
    ...
    // each cell to the declared type and cannot see this: a name resolves within its type, and
    // an owned type's names run across every owner, so a row naming another owner's entity
    // resolves and is wrong; that half is the check above's, which holds every such name. What is
