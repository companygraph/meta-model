...
export function instanceChecks({ files, core = "core", model = MODEL, fail }) {
  ...
  },
  {
    ...
    // the tree. An owner's schema may also have it say so in a table, a column declared as a
    // reference to the owned type, because the order the owned entities take is something a
    // folder cannot state. R16 already holds each cell to the declared type. What it cannot
    // see is whether the rows are the owner's own: a name resolves within its type, and an
    // owned type's names run across every owner, so a row naming another owner's entity
    // resolves and is wrong. Nor can it see order. Where the owned type declares a field that
    // references its own type, each entity names its successor, and the table says the same
    // thing a second time; the two are held to each other here, which is the half of that
    // writing rule a machine can read.
    //
    // No type is named. The check hangs on what the schemas declare: a type that owns another,
    // a column table of the owner whose reference is the owned type, and, for order, a field of
    // the owned type that is `ref → <itself>`. An owner declaring no such table is held to
    // nothing, and an owned type declaring no such field has no order to hold.
    name: "an owner's table of what it owns lists exactly that, in the order the owned give",
    rule: "R5",
    run() {
      for (const owner of TYPES.filter((t) => t.owns))
        for (const ownedType of owner.owns) {
          ...
          for (const name of ls(base) ?? []) {
            ...
            const at = table?.columns.indexOf(listing.column) ?? -1;
            // A section without its table, or a table without the column, is the column
            // check's finding; there are no rows here to hold.
            if (!table || at < 0) continue;
            const rows = table.rows.map((r) => (r[at] ?? "").trim()).filter(Boolean);

            // What this owner owns: the H1 of every entity in its own folder of the owned
            // type, with the field that names its successor where the type declares one.
            const mine = new Map();
            ...
            for (const f of (ls(folder) ?? []).filter((f) => f.endsWith(".md") && f !== "README.md")) {
              ...
              if (h1) mine.set(h1, successor ? fmScalar(frontmatterOf(body), successor) : null);
            }

            const where = `${file}: "## ${listing.section}"`;
            const seen = new Set();
            for (const row of rows) {
              if (seen.has(row)) fail(`${where} lists "${row}" twice; an owner lists each ${ownedType} it owns once`);
              seen.add(row);
              if (!mine.has(row))
                fail(`${where} lists "${row}", which is not one of its own: ${folder}/ holds no ${ownedType} of that name, and an owner lists the ${ownedType}s it owns and no other (R5)`);
            }
            ...
            // Order is held only between rows that are the owner's own; a row that is not has
            // been named above, and one finding per cause is enough.
            ...
            ordered.forEach((row, i) => {
              ...
              if (next === undefined) {
                if (says)
                  fail(`${where} ends on "${row}", whose \`${successor}\` says "${says}"; the last ${ownedType} leads nowhere, and only the last`);
              } else if (says !== next) {
