...
export function instanceChecks({ files, core = "core", model = MODEL, fail }) {
  ...
  },
  {
    ...
    // resolves and is wrong. `ownerListings` above says what is read and what is not.
    name: "an owner's table of what it owns lists exactly that",
    rule: "R5",
    run() {
      for (const { where, rows, mine, ownedType, folder } of ownerListings()) {
        ...
        for (const row of rows) {
          ...
          seen.add(row);
          if (!mine.has(row))
            fail(`${where} lists "${row}", which is not one of its own: it is the H1 of no ${ownedType} in ${folder}/, and an owner lists the ${ownedType}s it owns and no other (R5)`);
        }
