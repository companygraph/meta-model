...
export function instanceChecks({ files, core = "core", model = MODEL, fail }) {
  ...
  const joinsOf = (type) => {
    ...
    for (const [section, table] of columnTables)
      for (const r of table.rows) {
        const m = (r[3] ?? "").trim().match(LISTS);
        if (!m) continue;
        const byRow = table.rows.find((x) => clean(x[0]) === m[2]);
        lists.push({ section, column: clean(r[0]), target: pointsAt(r[2], "qualifier|ref"), field: m[1], by: m[2],
                     byTarget: byRow ? pointsAt(byRow[2], "ref|qualifier") : null });
      }

    const under = [];
    for (const r of blocks.find((b) => !b.section && !b.grouped && b.table)?.table.rows ?? []) {
      const m = (r[2] ?? "").trim().match(UNDER);
      const section = clean(r[0]).match(/^##\s+(.+)$/)?.[1];
      if (!m || !section) continue;
      under.push({ section, under: m[1].trim(), column: columnTables.has(section) ? referenceOf(columnTables.get(section)) : null,
                   underColumn: columnTables.has(m[1].trim()) ? referenceOf(columnTables.get(m[1].trim())) : undefined });
    }
    return { lists, under };
  };
  ...
  const listKindsOf = (type) => {
    ...
    for (const r of blocksOf(body).find((b) => !b.section && !b.grouped && b.table)?.table.rows ?? []) {
      const m = (r[2] ?? "").trim().match(LIST_KIND);
      const section = (r[0] ?? "").replace(/`/g, "").trim().match(/^##\s+(.+)$/)?.[1];
      if (m && section) out.push({ section, kind: m[2], shape: m[1] ?? null, required: (r[1] ?? "").replace(/`/g, "").trim() === "Yes" });
    }
    return out;
  };
