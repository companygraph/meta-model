...
export function upgradePlan({ core, skills = new Map(), tooling, tag, manifest, held, workflow, fetched = false, force = false }) {
  ...
  // Skills are moved only for an instance `init` gave them to, which its manifest shows. One
  // that has none made its own arrangements, as with a workflow it does not have, and an upgrade
  // does not hand it any.
  const hasSkills = Object.keys(was).some((path) => path.startsWith(SKILLS));
