...
A reader reads a file `IMAGE_FILE` matches as bytes and every other file as text, both exported from `companygraph-meta-model/instance` beside `parseInstance`; a site that publishes images calls `imagesOf(files, data, { sub, schemas })` after `parseInstance` and copies each result's `bytes` to `images/<to>`.
...
Setting that up and keeping it current is `bin/companygraph.mjs`'s job: one entry point with three subcommands, run from a release tag as `npx github:companygraph/meta-model#<tag> <command>`.
