...
Two checks are new, because of what R16 cannot see. A name resolves within its type, and an
owned type's names run across every owner, so a row naming another process's phase resolves and
is wrong. And R16 knows nothing of order. Both name no type and hang on what the schemas
declare: a type that owns another, a column table of the owner with a column declared
`ref → <the owned type>`, and, for order, a field of the owned type declared `ref → <itself>`,
which is what `gate-to` is. An owner that declares no such table is held to nothing, which is
every owner but a process today: a profile does not list its experiences, and its evidence
table's `Experience` column is a qualifier and is not read here.

**An owner's table of what it owns lists exactly that** fails a row that is not the H1 of one
of the owner's own entities, an owned entity no row names, and a row written twice. It cites
R5, the rule that makes what an owner owns a fact of the tree.

**An owner's table of what it owns is a table, in the order the owned give** fails a section
that holds no table the checks can read, two neighbors the successor field does not join, and a
last row whose entity still leads somewhere. It cites R9, which fixes what a schema may declare
and makes its writing rules part of the type; the order is a writing rule of both schemas.

The first of those three is what makes the rewrite enforced at all, and a review found it
missing. The checks' table reader refuses a separator row that is not plain dashes while the
parser reads such a table all the same and draws its edges, and a section left as the list of
links an older core asked for holds no table at all. With no rows to read, the first version of
the check said nothing in either case, and so did every other check: an instance could take the
release, rewrite nothing, and stay green. The successor is read with `fieldValues`, the reader
...
the ownership and once for the row. A consumer that renders a section's text finds the Phases
section's text empty, since a table is kept apart from the text, and reads the rows of its
table instead; the reference instance's site and the MCP server each render a process and are
checked against this when they take the release. A reader of the file on a forge loses the click from the
