...
The one list that names the types is `TYPES` in `lib/checks.mjs`; it gains `{ type: "brand", file: "brand.md" }` beside identity and vision. A singular type has no folder, so `init` writes no README for it and the folder-README rule does not reach it.
...
- The two favicons still carrying values older than the tokens get one sentence under `## Mark` saying the tokens are the master of the mark's colors. The fix to the files is a site change and stays out of the instance pull request.
