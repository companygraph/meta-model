...
The example instance under `example/` gains two kinds, Product at 10 and Company at 20. "How do I find out why a line is on my invoice?" and "Who split billing out of the monolith?" take Product; "Does Beacon Systems publish its revenue?" takes Company. The writing rule that a kind holds at least two questions is a writing rule and no check reads it, so the example's one Company question passes, and the plan adds a second example question only if the owner wants the rule shown rather than stated.
