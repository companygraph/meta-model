...
A release of meta-model that every consumer re-pins, as any new type asks, though no consumer's code changes. The order is meta-model's release; then each instance upgrades its core and is seeded; then the sites and the MCP hosts re-pin to the seeded commits as they do for any model change. companygraph.io's `/model/` page lists the core vocabulary term by term and gains a `kpi` row in that re-pin.
