...
Every field is written in vocabulary that exists — `ref`, `array of ref`, `enum`, `string` and a What and URL table — so the parser, the Obsidian plugin, the MCP server and the chat read the type from its schema and need no source change; the plugin still adopts it, as its own section says. `array of ref → kpi` is a type referencing its own, as a phase's `gate-to` does.

The checker's rules needed one. Writing `kpi.test.mjs`'s own fixtures found that "required frontmatter fields are present" (R9) had only ever read a field's key, never its value, so a required field left with no value — `unit: `, or a quoted empty string, `unit: ""` — passed as though it were written. It now fails the same way an absent key does, for every type this rule holds and not only `kpi`, which reaches every instance the moment it re-pins onto this release; the three instances were checked against it and hold no required field written blank.

The one list that names the types is `TYPES` in `lib/checks.mjs`, which states each type's folder rather than deriving it; it gains `{ type: "kpi", folder: "kpis", noun: "KPI" }`. The noun is new: `init` writes each folder's README from the folder's name, which would head this one "Kpis" and call its files "kpi", and an abbreviation is spelled the way it is read. A row without a noun reads as today. The same `noun` is also what `verify/check.mjs` reads for a schema's required H1 — `# KPI Schema`, where a row without one still gets the mechanical title, word-cased from the type id — and, while in that check, a branch that rejected any reference to a type whose id happens to end in "s" is removed as dead and wrong, first reached by `measures`'s `ref → process`. A search of the plugin, mcp-server, chat-server and design for type names found no other list: the site renderers select the few types they draw a page for, and none draws a KPI.
