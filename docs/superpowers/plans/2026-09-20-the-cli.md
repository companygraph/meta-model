...
Run: `node --test verify/instance-files.test.mjs`
Expected: FAIL, `Cannot find module '../lib/instance-files.mjs'`.
...
Run: `npm run verify && npm run test:instance-files && sh conventions/conventions-check`
Expected: all three exit 0. Commit `lib/instance-files.mjs`, `verify/instance-files.test.mjs`, `package.json`.
...
Run: `node --test verify/plan.test.mjs`
...
`"test:plan": "node --test verify/plan.test.mjs"` in `package.json`.
Run: `npm run verify && npm run test:plan && npm run test:instance-files && sh conventions/conventions-check`
Commit `lib/plan.mjs`, `verify/plan.test.mjs`, `package.json`.
...
Note for the implementer: `bin/check-instance.mjs` is one top-level body today, ending in
`process.exit(failures.length ? 1 : 0)`, and its `die` exits too. Move that body into an exported
`checkPath(root)` that returns the number of failures and throws its `die` messages as an `Error`
instead of exiting, and keep a direct-run entry at the foot —
`if (process.argv[1] && realpathSync(process.argv[1]) === fileURLToPath(import.meta.url))` — that
calls it, catches, prints and exits exactly as the file does now. Running the file directly must
behave identically, because the reusable workflow and every instance's CI call it by that path.
Both callers here treat the answer as a number, not as an exit: `check` exits with it, and
`upgrade` prints it and does not throw, since the files are already written.
...
Run: `npm run verify && npm run test:plan && npm run test:instance-files && npm run test:cli && npm run test:instance && npm run test:instance-checks && npm run test:rules && sh conventions/conventions-check`
Expected: every one exits 0. Commit the new files and `package.json`.
...
Run: `node --test verify/plan.test.mjs verify/cli.test.mjs`
Expected: FAIL, `upgradePlan is not a function`.
